...
Remote Everything 在电脑上运行一个轻量节点，把明确登记的本地 Web 应用送到 Android、iOS 或 HarmonyOS 客户端。它不改造原应用、不把数据搬到第三方云端：同网直接连接，异地则经过你自己的服务器或已有组网。
...
- **客户端**：设备凭据存入系统安全存储；每个应用由服务端在自己的 origin 上提供，Web 数据按 origin 隔离（同源策略），外部链接交给系统浏览器。
