...
文档地图：`README.md` = 给人类的入口；`AGENTS.local.md`（gitignore，不入库）= 本实例的服务器地址、指纹、巡检命令等；`PLAN.md`（gitignore，不入库）= 交付计划与路线图；`docs/` = 面向 Agent 的执行参考；`skills/` = 与具体 Agent 客户端无关的标准操作流程。
...
- Android：`cd android && gradlew.bat :app:assembleDebug`（需先跑 `configure-clients.ps1` 生成 `remote-everything.properties`）
...
- 密钥、令牌、PKCS#12、注册口令、真实配置：不进源码、不进提交、不进对话与日志。Android 连接参数只走被 gitignore 的 `android/remote-everything.properties`（BuildConfig 注入）。
