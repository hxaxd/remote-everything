...
2. 确认设备证书已批准并进入信任集合：`remote-everything-enroll list`。
3. 核对客户端用的是 PKCS#12 交付方案，不能只看证书是否已签发。
4. 检查客户端时间、证书有效期和信任链。

## 显示电脑未连接

1. 检查 Windows `RemoteEverything-Tunnel` 计划任务（frpc）与云端 `127.0.0.1:58628` 是否有隧道监听；frpc 日志在 `%LOCALAPPDATA%\RemoteEverything\logs\frpc.log`，frps 日志在云端 `journalctl -u frps`。
2. 检查 Windows `127.0.0.1:58627` 是否监听（`RemoteEverything-Apps` 任务）。
...
2. 检查 `RemoteEverything-Apps` 计划任务与 `%LOCALAPPDATA%\RemoteEverything\logs\<应用id>.log`、`control.log`（含启停动作与崩溃退避记录）。
...
3. 确认 `RemoteEverything-Apps` 直接运行 GUI 子系统构建的控制程序（`-H=windowsgui`）。
4. 修复后云端连续请求接口、Windows 高频采样验证，不要只凭肉眼判断。

## 注册一直停在「等待服务端批准」

1. 核对手机显示的 8 位审批码与服务器 `remote-everything-enroll list` 里的请求一致（设备名、时间）。
