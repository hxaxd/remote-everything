...
1. 本地验证（按改动范围）：分别在 `nodes/windows/control`、`server/gateway` 运行 `gofmt -l .` / `go vet ./...` / `go test ./...`；运行 Python `compile(...)` 语法检查；在 `clients/android` 运行 `gradlew.bat :app:assembleDebug`。全绿才继续。
