...
**先看这一条**：三端客户端正在按当前协议重写。本仓库里现有的客户端代码说的是早先的协议（邀请里带版本、安装 ID 和模式），和现在的网关对不上——装上去也配不成对。重写完成之前**不要交付客户端**，也不要把下表里的旧版资产装到用户手机上；这时能给用户的说法只有「这一轮还没有可用的客户端」。下表是重写完成后仍然适用的取得方式。
