...
隧道客户端证书到期前，停节点 FRPC，使用新的空输出目录执行 `tunnel renew`，通过加密管理通道送到节点并再次执行节点 `init --bootstrap`。节点只在 installation ID、控制令牌、FRPS token 与隧道 CA 全部匹配时写入以新指纹命名的客户端证书/私钥；按 CLI 返回路径重渲染、重启并验证 FRPC 后，删除旧身份文件和两端 bundle 副本。该操作不改变端口、设备 CA 或移动客户端授权。
