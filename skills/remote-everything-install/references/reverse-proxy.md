...
**部署前提**：DNS 需要一条 `*.<域名>` 通配 A/AAAA 记录，指向入口所在主机。应用主机是两个 label（`<应用>.<节点前缀>`）加域名，但 DNS 通配记录按 RFC 4592 会覆盖没有更近节点存在的多级名字，所以一条 `*.<域名>` 就够；Caddy 侧则是 `https://*.*.<域名>`（Caddy 的 `*` 只匹配一个 label）。证书不是通配证书：每个应用主机一张，按需签发，因此不发多级通配证书也能工作。
