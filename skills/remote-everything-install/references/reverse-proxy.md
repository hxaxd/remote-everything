...
7. 设备上游分为独立连接通道：`/__remote_everything*` 目录与控制固定最多 2 个连接，其余网页数据固定最多 4 个连接。FRP v0.70.0 在默认 `poolCount=0` 时服务端工作连接通道容量为 10；总并发收敛到 6，并保证网页资源突发不能占用目录通道，避免 `work connection pool is full` 或目录探活等满 7 秒后误报节点离线。
