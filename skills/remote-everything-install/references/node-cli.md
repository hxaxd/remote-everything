...
代理链路只剥离 `X-Remote-Everything-Client-Fingerprint`、`X-Remote-Everything-Control-Token` 与 `X-Remote-Everything-Node` 三个保留头（名字都在 `internal/proxysecurity`，网关那一跳与节点这一跳用的是同一份列表）；应用自身的 `Authorization` 与其余头部原样透传。
