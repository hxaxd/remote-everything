...
- `scripts/cloud/frps-healthcheck/`：public 云端 frps 兜底探活，仅 Linux 服务器侧安装。`frps-healthcheck.sh` 与 `remote-everything-frps-healthcheck.service` 同样是 `{{NAME}}` 占位符模板，但不走 `render.py`；由该目录 `install.sh --installation-id <ID> --node-tunnel-listen <init 输出的 node_tunnel_listen>` 渲染三处占位符（脚本实际路径、installation_id、探活 URL）并安装启用 timer。systemd 不展开 ExecStart 可执行名中的环境变量，因此路径只能在安装期渲染，不得改成 `$VAR` 形式。
