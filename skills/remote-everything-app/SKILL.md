...
**只接入可信、无恶意的应用及插件；这是部署前提。** 当前系统不提供不可信应用之间的完整安全隔离。系统浏览器中的 Web 客户端共享浏览器 Cookie 范围：专门针对 Remote Everything 设计的恶意应用，可能跨应用、跨节点读取部分 Cookie 数据或干扰会话。公网网关限制服务器下发 Cookie 的域范围，但无法拦截网页脚本直接写入父域 Cookie；LAN 的不同端口也不分隔 Cookie。原生客户端另有按网关、节点、应用划分的网页会话隔离，其存储隔离不改变按节点授权的规则，也不构成恶意应用安全沙箱。不得把独立应用地址、设备认证或常规功能验证当作应用无恶意或完整隔离的证明。
